...
if (!validEmail.test(email)) return res.status(400).send({ status: false, message: "Invalid name." })
const validMobile = (/^(\+\d{1,3}[- ]?)?\d{10}$/)

if (!validMobile.test(mobile)) return res.status(400).send({ status: false, message: "Invalid name." })
